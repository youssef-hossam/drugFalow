...
class _OnBoardingScreenState extends State<OnBoardingScreen> {
  ...
  List<OnboardingData>onBoardingList =[
    OnboardingData(title: "كل مخازن الأدوية في مكان واحد",
        description: "تصفح مئات المنتجات من أكثر من مخزن، وقارن الأسعار واختر الأنسب لصيدليتك بضغطة واحدة"),
    OnboardingData(title: "طلب واحد، موردين متعددين",
        description: "أضف منتجات من مخازن مختلفة في سلة واحدة، ووفّر وقتك وجهدك في إدارة المشتريات اليومية"),
    OnboardingData(title: "خصومات حصرية ومتابعة لحظية", description: "استفد من كوبونات الخصم المتاحة، وتابع حالة طلباتك من لحظة التأكيد حتى التسليم.")
  ];
  @override
            ...
            BlocBuilder<OnboardingCubit, OnboardingState>(
                builder: (context, state) {
                  return OnboardingDescriptionWidget(
                    title: onBoardingList[context.read<OnboardingCubit>().pageInd!].title,
                    description: onBoardingList[context.read<OnboardingCubit>().pageInd!].description,);
                }),
            SizedBox(height: context.height/60,),
            BlocBuilder<OnboardingCubit, OnboardingState>(
              builder: (context, state) {
            ...
            width: double.infinity,
                  ...
                  child:   BlocBuilder<OnboardingCubit,OnboardingState>(
                            builder: (context, state) {
                              return CustomButton(
                                btnTitle:
                                (context.read<OnboardingCubit>().controller.page == 0 ||
                                    context.read<OnboardingCubit>().controller.page == 1
                                )?  tr("next") : tr("login"),
                                onPressed: ()async{
                                 (context.read<OnboardingCubit>().controller.page == 0  )?
                                  context.read<OnboardingCubit>().controller.animateToPage(1,
                                      duration: Duration(milliseconds: 500)
                                      , curve: Curves.easeInOutExpo) :
                                  (context.read<OnboardingCubit>().controller.page == 1)?
                                  context.read<OnboardingCubit>().controller.animateToPage(2,
                                      duration: Duration(milliseconds: 500)
                                      , curve: Curves.easeInOutExpo) :
